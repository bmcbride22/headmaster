...
SimpleForm.setup do |config|
  ...
  config.button_class = 'my-2 bg-violet-500 hover:bg-violet-700 text-white font-bold text-sm py-2 px-4 rounded'
  ...
  config.wrappers :vertical_form, tag: 'div', class: 'mb-4' do |b|
    ...
    b.use :input,
          class: 'shadow appearance-none border border-gray-300 rounded w-full py-2 px-3 bg-white focus:outline-none focus:ring-0 focus:border-violet-500 text-gray-400 leading-6 transition-colors duration-200 ease-in-out', error_class: 'border-red-500', valid_class: 'border-green-400'
  ...
  config.wrappers :vertical_boolean, tag: 'div', class: 'mb-4 flex items-start', error_class: '' do |b|
    ...
    b.wrapper tag: 'div', class: 'flex items-center h-5' do |ba|
      ba.use :input,
             class: 'focus:ring-2 focus:ring-violet-500:focus ring-offset-2 h-4 w-4 text-violet-600 border-gray-300 rounded'
    end
  ...
  config.wrappers :vertical_collection, item_wrapper_class: 'flex items-center',
                                        item_label_class: 'my-1 ml-3 block text-sm font-base text-gray-400', tag: 'div', class: 'my-4' do |b|
    b.use :html5
    b.optional :readonly
    b.wrapper :legend_tag, tag: 'legend', class: 'text-sm font-base text-gray-600',
                           error_class: 'text-red-500' do |ba|
      ba.use :label_text
    end
    b.use :input,
          class: 'focus:ring-2 focus:ring-violet-500 ring-offset-2 h-4 w-4 text-violet-600 border-gray-300 rounded mr-2 transition-colors duration-200 ease-in-out', error_class: 'text-red-500', valid_class: 'text-green-400'
